...
class Bowling:
    ...
    def get_bowling_stats():
        ...
        with connection.cursor(dictionary=True) as cursor:
            cursor.execute("""
                SELECT Bowling.*, Player.Player_Name
                FROM Bowling
                JOIN Player ON Bowling.Player_ID = Player.Player_ID
            """)
    ...
    def get_bowling_scorecard(match_id, inning_number):
        ...
        query = """
            SELECT Bowling.Player_ID, Player.Player_Name, 
                   Bowling.Overs_Bowled, Bowling.Balls_Bowled, Bowling.Runs_Conceded, Bowling.Wickets_Taken
            FROM Bowling
            JOIN Player ON Bowling.Player_ID = Player.Player_ID
            WHERE Bowling.Match_ID = %s AND Bowling.Inning_Number = %s
        """
    ...
    def get_bowling_stats_by_player(match_id, player_id):
        ...
        with connection.cursor(dictionary=True) as cursor:
            cursor.execute("""
                ...
                FROM Bowling
                JOIN Player ON Bowling.Player_ID = Player.Player_ID
                WHERE Bowling.Match_ID = %s AND Bowling.Player_ID = %s
            """, (match_id, player_id))
    ...
    def add_bowling_stats(bowling_data):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                """,
                (
                    bowling_data['Player_ID'], bowling_data['Match_ID'], bowling_data['Inning_Number'],
                    bowling_data['Overs_Bowled'], bowling_data['Balls_Bowled'], bowling_data['Runs_Conceded'],
                    bowling_data['Wickets_Taken'], bowling_data['Maiden_Overs']
                )
            )
            connection.commit()
    ...
    def delete_bowling_stats(match_id, player_id, inning_number):
        connection = get_db_connection()
        with connection.cursor() as cursor:
            cursor.execute("DELETE FROM Bowling WHERE Match_ID = %s AND Player_ID = %s AND Inning_Number = %s", 
                           (match_id, player_id, inning_number))
            connection.commit()
