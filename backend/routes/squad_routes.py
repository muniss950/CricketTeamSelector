...
def add_squad_entry():
    ...
    Squad.add_squad_entry(data['Player_ID'], data['Team_ID'])
    return jsonify({"message": "Squad entry added successfully"}), 201

@squad_bp.route('/squad/<int:player_id>/<int:team_id>', methods=['GET'])
def get_squad_entry(player_id, team_id):
    squad_entry = Squad.get_squad_entry(player_id, team_id)
    if squad_entry:
        return jsonify(squad_entry)
    else:
        return jsonify({"error": "Squad entry not found"}), 404

@squad_bp.route('/squad/<int:team_id>', methods=['GET'])
def get_squad_for_team(team_id):
    squad_entry = Squad.get_squad_by_team(team_id)
    if squad_entry:
        return jsonify(squad_entry)
    else:
        return jsonify({"error": "Squad entry not found"}), 404
@squad_bp.route('/squad/<int:player_id>/<int:team_id>', methods=['PUT'])
def update_squad_entry(player_id, team_id):
    data = request.get_json()
...
@squad_bp.route('/squad/<int:player_id>/<int:team_id>', methods=['DELETE'])
def delete_squad_entry(player_id, team_id):
    Squad.delete_squad_entry(player_id, team_id)
